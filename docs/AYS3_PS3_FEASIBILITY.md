...
the prior project's know-how (iOS JIT via a debugger handshake, library/cover UX) — as a
**separate "AYS3" application**, not merged into the prior project.

## Why AYS3 must be a SEPARATE app (license constraint)

- **RPCS3** is **GPL-2.0-only**.
- **the prior project**  is **GPL-3.0-or-later**.
- GPL-2.0-only and GPL-3.0 are **mutually incompatible**: the two cannot be
  linked into one binary. "Put the PS3 core inside the prior project" = one binary = license
...
3. **JIT.** RPCS3 maps large executable (W^X) regions for the PPU/SPU
   ...
   appetite far exceeds a PS2 emulator's, compounding the RAM wall.
...
Only if 1–3 pass: re-implement an the prior project-style UI (independent, not imported), a
...
- No on-device verification is possible in this environment (no Mac, no test
  device); final confirmation stays with the user via sideload, as with the prior project.
